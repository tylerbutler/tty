...
import tty/resolve_color_level
...
fn resolve(
  ...
  case resolve_color_level.resolve_color_level(is_tty: is_tty, env: env) {
    0 -> NoColor
    1 -> Basic
    2 -> Ansi256
    _ -> TrueColor
...
pub fn no_color_disables_everything_test() {
  resolve(is_tty: True, env: env([#("NO_COLOR", "1"), #("FORCE_COLOR", "3")]))
...
pub fn colorterm_24bit_yields_truecolor_test() {
  resolve(is_tty: True, env: env([#("COLORTERM", "24bit")]))
  |> expect.to_equal(TrueColor)
}
...
pub fn term_with_256_yields_ansi256_test() {
  resolve(is_tty: True, env: env([#("TERM", "xterm-256color")]))
  |> expect.to_equal(Ansi256)
}

pub fn ci_yields_basic_test() {
  resolve(is_tty: True, env: env([#("CI", "true"), #("TERM", "xterm")]))
  |> expect.to_equal(Basic)
}
...
pub fn ci_rule_is_checked_before_default_basic_test() {
  ...
  resolve(is_tty: True, env: probed_env([#("CI", "true"), #("TERM", "vt100")]))
  |> expect.to_equal(Basic)
...
pub fn ci_does_not_override_no_color_test() {
  resolve(is_tty: True, env: env([#("CI", "true"), #("NO_COLOR", "1")]))
  |> expect.to_equal(NoColor)
}

pub fn ci_does_not_override_force_color_test() {
  resolve(is_tty: True, env: env([#("CI", "true"), #("FORCE_COLOR", "2")]))
  |> expect.to_equal(Ansi256)
}

pub fn ci_does_not_override_non_tty_test() {
  resolve(is_tty: False, env: env([#("CI", "true")]))
  |> expect.to_equal(NoColor)
}

pub fn ci_does_not_override_term_dumb_test() {
  resolve(is_tty: True, env: env([#("CI", "true"), #("TERM", "dumb")]))
  |> expect.to_equal(NoColor)
}
...
pub fn ci_distinguishes_from_default_test() {
  ...
  let with_ci = resolve(is_tty: True, env: env([#("CI", "true")]))
  let without_ci = resolve(is_tty: True, env: env([]))
  expect.to_equal(with_ci, Basic)
  expect.to_equal(without_ci, NoColor)
}

pub fn force_color_false_disables_test() {
  resolve(is_tty: True, env: env([#("FORCE_COLOR", "false")]))
  |> expect.to_equal(NoColor)
}

pub fn force_color_true_enables_basic_test() {
  resolve(is_tty: False, env: env([#("FORCE_COLOR", "true")]))
  |> expect.to_equal(Basic)
}
